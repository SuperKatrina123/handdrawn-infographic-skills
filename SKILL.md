...
- [ ] Step 1: Parse input & pick layout archetype ⚠️ REQUIRED
  ...
  - [ ] 1.4 Scan for abstract concept keywords → map each to a visual metaphor (see `references/abstract-metaphors.md`)
...
**Scaffolding-word discipline (critical — Gemini leaks these into the image):**

Gemini treats uppercase / named layout words in the prompt as visible labels and draws them onto the canvas. Prevent this three ways:

1. **Describe position, don't name the zone.** Use spatial language in the prompt body:
   - ❌ `SECTION 1 — "The Crowded Vector Space" (upper 65%):`
   - ✅ `In the upper two-thirds of the canvas:` / `Below that, filling the lower third:`
   Keep markdown headings (`###`, `**SECTION 1**`) only in your *internal notes* — strip them before the prompt goes to Gemini.

2. **Ban scaffolding vocabulary in the prompt body.** The following words must NOT appear in the text sent to Gemini (unless they're inside a card's labeled content in the element list): `SECTION`, `Section 1`, `Section 2`, `PART`, `ZONE`, `PANEL`, `CARD`, `BANNER`, `COLUMN`, `ROW`, `UPPER`, `LOWER`, `LEFT SIDE`, `RIGHT SIDE`, `TITLE BLOCK`, `HEADER`, `FOOTER`, aspect-ratio tokens like `16:9`, percentage tokens like `65%`. Rewrite into plain-English spatial description.

3. **Close with an explicit whitelist.** The final block of every prompt must list every string that is allowed to appear as rendered text in the image:

```
Allowed rendered text (exhaustive — do NOT draw any other text):
- "{TITLE}" (if used)
- "{SUBTITLE}" (if used)
- every card label, caption, speech-bubble quote, and leader-line label from the element list above, verbatim
- every fake tool name / code string listed in the element list, verbatim
```

And in the "Do not use" list:

```
Do not render any layout scaffolding words as visible text in the image —
including SECTION, PART, ZONE, PANEL, CARD, BANNER, COLUMN, ROW, UPPER,
LOWER, LEFT, RIGHT, TITLE, SUBTITLE, HEADER, FOOTER, percentage numbers,
aspect-ratio tokens (16:9, 4:3), or any word describing layout rather than
content. Only strings listed in the "Allowed rendered text" block may
appear in the image.
```
...
- **Don't label what you can draw.** If a concept has a canonical visual in `references/abstract-metaphors.md` (迷路 = fork-in-the-road, 稀释 = spotlight-on-crowd, 碰撞 = twin-keys, 挤压 = overstuffed suitcase, 失效 = scattered index cards, 漂移 = drifting boat…), the drawing must carry the meaning. Do NOT write the abstract word as a card title. Only an italic one-line caption describing the *consequence* is allowed under the drawing.
- **Leaking layout scaffolding words into the image.** Gemini will render `SECTION 1`, `PANEL`, `Upper 65%`, `16:9`, `COLUMN` as visible text on the canvas if you write them in the prompt body. Strip every scaffolding word from the prompt sent to Gemini — use spatial prose (`in the upper two-thirds`, `below that`) instead. Every prompt must end with an explicit "Allowed rendered text" whitelist so Gemini has a closed set.
...
- [ ] Prompt body contains **zero** scaffolding words (SECTION / PANEL / ZONE / CARD / BANNER / COLUMN / ROW / UPPER / LOWER / percentage / aspect-ratio) — only spatial prose ("in the upper two-thirds", "below that")
- [ ] Prompt ends with an explicit "Allowed rendered text" whitelist enumerating every string that may appear in the image
- [ ] Every abstract concept keyword from the input has been mapped to a visual metaphor (not written as a text label)
...
- [ ] Prompt's "do not use" list includes the scaffolding-words negation
